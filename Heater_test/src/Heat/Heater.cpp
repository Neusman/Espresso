// Heater.cpp
#include "Heater.h"
#include <Arduino.h>  // FIX: Add this for pinMode, digitalWrite, millis, micros
...
void Heater::setup() {
    // FIX: Validate heaterPin before using it
    if (heaterPin > 39) {
        ESP_LOGE(LOG_TAG, "Invalid heaterPin: %d. Must be 0-39.", heaterPin);
        return;
    }
    pinMode(heaterPin, OUTPUT);  // Now safe
    setupPid();
...
void Heater::loopPid() {
    ...
    if (combinedKff > 0.0f && pumpFlowRate && *pumpFlowRate > 0.01f && valveStatus && *valveStatus != 0) {
        float currentFlowRate = *pumpFlowRate;
...
void Heater::loopAutotune() {
    ...
    ESP_LOGI(LOG_TAG, "Autotuning finished: Kp=%.4f, Ki=%.4f, Kd=%.4f, Kff=%.4f",
             autotuner->getKp() * 1000.0f, autotuner->getKi() * 1000.0f, autotuner->getKd() * 1000.0f, kffFromWattage);
...
void Heater::plot(float optimumOutput, float outputScale, uint8_t everyNth) {
    if (plotCount >= everyNth) {
        plotCount = 1;
        ESP_LOGI(LOG_TAG, "PID Plot: output=%.2f, input=%.2f, setpoint=%.2f",
                 optimumOutput * outputScale, temperature, setpoint);
    } else {
        plotCount++;
    }
}
