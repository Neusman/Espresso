...
void Display::drawWelcomeScreen()
{
	...
	oled->drawStringMaxWidth(128,10,64, "Welcome");
	oled->setTextAlignment(TEXT_ALIGN_RIGHT);
	oled->setFont(ArialMT_Plain_10);
	oled->drawStringMaxWidth(64,50,64, "Fiers Espresso Controller");
	oled->display();
}
...
void Display::drawConnectionScreen()
{
	...
	oled->setTextAlignment(TEXT_ALIGN_CENTER);
	oled->setFont(ArialMT_Plain_10);
	oled->drawStringMaxWidth(64,10,128, "drawConnectionScreen");
...
void Display::drawTrigger()
{
	...
	oled->setTextAlignment(TEXT_ALIGN_CENTER);
	oled->setFont(ArialMT_Plain_16);
	oled->drawStringMaxWidth(64,10,128, "Triggered");
	oled->display();
}

void Display::drawNOTtrigger()
{
	oled->clear();
	oled->setTextAlignment(TEXT_ALIGN_CENTER);
	oled->setFont(ArialMT_Plain_16);
	oled->drawStringMaxWidth(64,10,128, "NOT Triggered");
	oled->display();
}
